...
import { TEMPO_PRESETS } from "./discover-view.utils";
import type { DiscoverPagination, DiscoverSong } from "./discover-view.types";
...
export function useDiscoverBrowse(
  ...
  const skipInitialFetch = useRef(
    !!opts.initialSongs && active,
  );

  const [songs, setSongs] = useState<DiscoverSong[]>(opts.initialSongs ?? []);
  const [pagination, setPagination] = useState<DiscoverPagination>(
    opts.initialPagination ?? { page: 1, totalPages: 1, total: 0, hasMore: false },
  );
  const [sortBy, setSortBy] = useState(opts.initialSortBy || "newest");
  const [tag, setTag] = useState(opts.initialTag || "");
  const [mood, setMood] = useState(opts.initialMood || "");
  const [tempoPreset, setTempoPreset] = useState(opts.initialTempo || "");
  const [loading, setLoading] = useState(!opts.initialSongs);
  ...
  useEffect(() => {
    ...
    setSongs([]);
    fetchSongs(1, sortBy, tag, mood, tempoRange?.min ?? null, tempoRange?.max ?? null);
