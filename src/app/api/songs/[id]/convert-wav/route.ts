...
export async function POST(
  ...
  try {
    ...
    if (song.generationStatus !== "ready") {
      return NextResponse.json({ error: "Song must be fully generated before converting to WAV." }, { status: 400 });
    }
    ...
    if (!hasApiKey) {
      ...
      if (!song.sunoJobId) {
        return NextResponse.json({ error: "Song has no Suno task ID for WAV conversion." }, { status: 400 });
      }
