...
export function LibraryBatchActionBar({
  ...
  return (
    <div className="fixed bottom-20 md:bottom-4 left-2 right-2 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-40 flex items-center gap-2 px-4 py-3 bg-gray-900 dark:bg-gray-800 text-white rounded-2xl shadow-2xl border border-gray-700 animate-slide-in overflow-x-auto">
      ...
      >
        <Icon icon={Heart} fill="currentColor" className="w-4 h-4" />
      ...
      <div className="relative" ref={batch.batchTagMenuRef}>
        ...
        {batch.showBatchTagMenu && (
          <div className="absolute bottom-full mb-1 left-0 w-48 bg-surface border border-border rounded-xl shadow-lg z-50 overflow-hidden max-h-60 overflow-y-auto">
            {availableTags.length === 0 ? (
              <p className="px-4 py-3 text-sm text-secondary">No tags yet</p>
            ) : (
              availableTags.map((tag) => (
                <button
                  key={tag.id}
                  onClick={() => batch.handleBatchTag(tag.id)}
                  className="w-full text-left px-4 py-3 text-sm text-primary hover:bg-surface-hover transition-colors border-b last:border-b-0 border-border flex items-center gap-2"
                >
                  <span
            ...
            {batch.batchPlaylists.length === 0 ? (
              ...
              batch.batchPlaylists.map((pl: PlaylistOption) => (
                <button
                  ...
                  className="w-full text-left px-4 py-3 text-sm text-primary hover:bg-surface-hover transition-colors border-b last:border-b-0 border-border"
                >
                  {pl.name}
      ...
      <div className="relative flex-shrink-0" ref={batch.batchDownloadFormatMenuRef}>
        <div className="flex items-stretch">
          ...
          >
            <Icon icon={ChevronDown} className={`w-3 h-3 transition-transform duration-150 ${batch.showBatchDownloadFormatMenu ? "rotate-180" : ""}`} />
          </button>
        </div>
