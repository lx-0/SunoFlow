...
function StatCardSkeleton() {
  return (
    <div className="bg-surface border border-border rounded-xl px-4 py-5 animate-pulse">
      <div className="h-3 w-16 bg-surface-raised rounded" />
      <div className="h-7 w-12 bg-surface-raised rounded mt-2" />
    </div>
  );
}
...
function TopTagsSkeleton() {
  return (
    <div className="flex flex-wrap gap-2 animate-pulse">
      {[1, 2, 3, 4, 5].map((i) => (
        <div key={i} className="h-7 rounded-full bg-surface-raised" style={{ width: `${50 + i * 12}px` }} />
      ))}
    </div>
  );
}
...
function StatCard({ label, value, href }: { label: string; value: string; href?: string }) {
  const content = (
    <div className={`bg-surface border border-border rounded-xl px-4 py-5 ${href ? "hover:border-violet-400 transition-colors" : ""}`}>
...
function UsageChart({ dailyCounts, limit }: { dailyCounts: { date: string; count: number }[]; limit: number }) {
  ...
  return (
    <div>
      <h3 className="text-base font-semibold text-primary mb-3">Generation history (7 days)</h3>
      <div className="bg-surface border border-border rounded-xl p-4">
          ...
          {dailyCounts.map((d) => {
            ...
            return (
              <div key={d.date} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                <span className="text-xs text-secondary font-medium">{d.count}</span>
                <div
                ...
                />
                <span className={`text-xs ${isToday ? "text-violet-600 dark:text-violet-400 font-semibold" : "text-secondary"}`}>
                  {dayName}
                </span>
              </div>
            );
        ...
        <div className="mt-2 pt-2 border-t border-border flex items-center justify-between text-xs text-secondary">
        ...
        <div className="flex items-end gap-1.5 h-32">
          {[1, 2, 3, 4, 5, 6, 7].map((i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
              <div className="w-full bg-surface-raised rounded-t-md" style={{ height: `${20 + i * 8}%` }} />
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      ...
      <div data-tour="welcome">
        ...
        <p className="text-secondary text-sm mt-1">Your Suno music hub</p>
      </div>
      ...
      {loading && !stats ? (
        <div>
          <h3 className="text-base font-semibold text-primary mb-3">Top styles</h3>
          <TopTagsSkeleton />
        </div>
      ...
      {/* Recent songs */}
      <div>
        <h3 className="text-base font-semibold text-primary mb-3">Recent songs</h3>
        {loading && !stats ? (
          <div className="bg-surface border border-border rounded-xl overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
            <RecentSongSkeleton />
            <RecentSongSkeleton />
            <RecentSongSkeleton />
          </div>
          ...
          <div className="bg-surface border border-border rounded-xl p-6 text-center">
            <Icon icon={Music} className="w-10 h-10 mx-auto text-muted mb-3" fill="currentColor" />
      ...
      {(dailyLoading || (dailySongs && dailySongs.length > 0)) && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Icon icon={Sparkles} className="w-4 h-4 text-violet-500" fill="currentColor" />
            <h3 className="text-base font-semibold text-primary">Daily discovery</h3>
          </div>
          <div className="bg-surface border border-border rounded-xl overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
            {dailyLoading ? (
              <>
