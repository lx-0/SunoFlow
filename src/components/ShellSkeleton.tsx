...
export function ShellSkeleton({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col min-h-screen max-w-md mx-auto">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-10 bg-surface border-t border-border max-w-md mx-auto">
        <div className="flex items-center justify-around h-16">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="flex flex-col items-center gap-1 px-3 py-2">
              <div className="w-6 h-6 rounded bg-surface-raised" />
              <div className="w-8 h-2 rounded bg-surface-raised" />
            </div>
          ))}
        </div>
      </nav>
