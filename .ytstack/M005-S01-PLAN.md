...
- [x] T05 -- Completion hook: on generation completion the song joins the session playlist and is flagged for host-queue append (extend the `src/lib/songs/lifecycle.ts` seam — direct prisma updates are a smell); unit tests on the seam.
