...
- [ ] T01 -- Prisma schema + migration: `JamSession` model (host, linked playlist, share token, budget total/used, status open/closed) plus a session-entry linkage that ties generated songs to the session with prompt text + guest display name; migration applies cleanly on a throwaway DB.
