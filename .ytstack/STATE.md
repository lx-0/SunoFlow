...
project: SunoFlow
slug: SunoFlow
last_updated: 2026-07-22T23:30:00Z
...
**Status:** M005 — S01 complete, roadmap reassessed (fits, outcome A). **M005 COMPLETE incl. S04 native host surface** (operator request post-GATE): the RN app now has a "Jam Session" sidebar entry, a list/create screen (name, link-name, budget, duration chips) and a host console (5s poll, budget countdown, QR via react-native-qrcode-svg + native share sheet, veto, end-session Alert). Server 403s non-studio (the app does not know the tier). RUNTIME-UNVERIFIED — ships to the device together with the section-back-button fix on the next `pnpm release`. PLUS party display (operator request): fullscreen landscape-capable presentation mode (Modal, big Geist Extrabold + magenta, giant QR, budget countdown, now-playing, pulsing LIVE dot via plain RN Animated, keep-awake). NOTE: expo-screen-orientation + expo-keep-awake are NEW NATIVE MODULES and app.json orientation flipped to default with an initial PORTRAIT_UP lock via the screen-orientation plugin — the next pnpm release is a required FULL native build (which pnpm release does anyway). Remaining acceptance: the real party test. **Web round `d1a19425` LIVE (CI green, Railway SUCCESS, /party 307→login for anon verified):** new `/party` index (session list + create form), "Jam" nav item under Create (visible to all, server 403s non-studio), Favorites back in the nav under My Music (partial Phase-2 revert, DECISIONS 2026-07-22), and server-side auto-slug (title → kitchen-party-x3f2, silent collision retry; only custom slugs 409). **M004 GATE PASSED 2026-07-22**: user ran `pnpm release` (fresh native build) and verified on-device — background audio through a 10+ min lock (S03's exit criterion), tabs navigation, playback edge cases (auto-advance, rapid skips, seek-after-switch), lyrics highlight incl. the new sync trigger, UI wave, VoiceOver spot-check: all good — ONE finding: section back-buttons (mashup/personas/inspire …) returned to Library regardless of the entry tab; root cause chain: goToSection navigated BARE shared-group hrefs → v1 fix group-qualified the path (c22fae6a) — PROVEN INEFFECTIVE on device (Jam entry present = fix bundle active, Back still → Library) → v2 (`f0913234`) dispatches NAVIGATE by name on the ACTIVE tab's child stack (target = its state key, the jumpToTab idiom; NAVIGATION.md rule 1b). v2 is RUNTIME-UNVERIFIED until the next pnpm release; on-device quick check: the tab-bar highlight must NOT jump to Library while a section is open. M004-ROADMAP flipped S01-S03 [x] (they had never been maintained). Still open in M004: S04 remainder (native waveform, playlist drag-reorder — needs Reanimated wiring: babel plugin + ANOTHER native rebuild) and S05 TestFlight (blocked on a paid Apple Developer account, user decision). Also unblocked now: mobile archive-tile nav (JS-only). /api-docs fixed 2026-07-22 (`b52b827d`, corejs noop-catch-all vs class polyfills). S01 learnings for S02: session playlist shows in the host's normal playlist list (presentation call in S02-T01); nowPlaying freshness depends on PlaybackState persist cadence (verify/heartbeat in S02-T03). M004 remains open on its user-side GATE (below); agent-side M004 work is blocked on that device pass.
