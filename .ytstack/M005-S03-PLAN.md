...
- [ ] T01 -- Guest session page (`/jam/[token]`, public tokened route): now-playing + queue with pending prompt cards and guest names + budget countdown; mobile-first; polling.
- [ ] T02 -- Prompt composer: free text + vibe chips, optimistic pending card, visible error states for rate limit / budget exhausted / session closed (no silent branches).
- [ ] T03 -- Guest nickname: one-time capture (localStorage), attached to prompts and shown on cards.
