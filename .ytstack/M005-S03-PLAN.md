...
- [x] T04 -- DESCOPED (2026-07-22): guest reactions need a schema change (SongReaction.userId is a required FK); not v1-worthy — v2 candidate via a guestKey-keyed table. See T04-SUMMARY.
...
- [x] T05 -- Full-path E2E: host context creates a session, a second (guest) browser context joins via token URL, pushes a prompt, the card is visible in both contexts, mocked completion makes the song playable in the session playlist.
