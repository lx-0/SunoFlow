...
- **Jam: the session operator can queue their own prompts** (2026-07-25, `1f20cdc7`). The host had no prompt input at all, on web or mobile — running a party meant scanning your own QR code and joining as a guest. `pushJamPrompt` is split into a shared `pushPrompt()` with an `asHost` flag; the host path resolves the session by `(id, hostUserId)` instead of by share token, so ownership is verified in the domain layer rather than trusted from the caller. Identical pipeline — same account, same Suno key, same atomic budget reservation, so "songs left" stays honest — but the per-guest open-prompt cap is skipped: it exists to stop one phone monopolising the party and is not a constraint on the person running it. Host entries carry a stable per-session `guestKey` (`host:<id>`). New authenticated route `POST /api/jam-sessions/[id]/entries`, mirroring the existing `DELETE …/entries/[entryId]`. Composers in `PartyHostView` and the native jam-session screen; both take the server's entry card into the live queue rather than an optimistic row a poll could contradict. Note the lookup moved `findUnique` → `findFirst` (`(id, hostUserId)` is not a unique index), so the existing prompt-test mocks were migrated.
- **Jam: "Optimize" turns a rough idea into a fuller song prompt** (2026-07-25, `60363f4a`). Guests and the host get an Optimize button next to the prompt input. Same trust model as the prompt push — the share token is the auth and the call runs on the host's Suno key — so the host console deliberately calls the same tokened route instead of the authenticated style-boost endpoint. It does **not** decrement `budgetUsed` (that counter is denominated in songs and an optimize is not a song) but does require budget headroom, so a used-up party cannot keep burning host credits; the path gets its own `jam_optimize` IP bucket (max 10) since it is an unauthenticated surface spending someone else's credits. Suno can exceed the 500-char description cap, so the result is truncated — otherwise the guest is handed a prompt they cannot submit. Both composers keep the pre-optimize text and offer **Undo**.
- **Jam: Optimize learns from what the party is reacting to** (2026-07-25, `fa6bb705`). The rewrite is now steered by the session's own reaction, read entirely from data the session already produces — no new tables, no added tracking. Positive signal is **playback**, not successful generation: every accepted prompt reaches `ready`, so `ready` carries no preference information, while a song the room actually heard is the closest thing to a vote it casts (read from the HOST's `PlayHistory`, since the party plays on their queue). Negative signal is the host's explicit veto; failed generations are excluded, an upstream error says nothing about taste. Each list caps at 8 so one outlier cannot dominate. With a signal present the rewrite goes through the existing `generateText` helper (which already owns the wall-clock deadline and content-reject filter); without one — a fresh party, the first prompt of the night — or when the model returns nothing usable, it falls back to Suno's stateless boost, so the button can never be worse than before. Model output is unwrapped (fences/quotes) before use. The signal is deliberately session-scoped and resets with the party; cross-session learning from the host's own accept/edit/undo history is the next layer and needs its own storage decision.
...
- **Party Mode: native host surface + fullscreen party display in the iOS app** (2026-07-22, `4a3d6297`…`f26c05bc`; runtime verification pending the next `pnpm release`). The RN app gets a "Jam Session" sidebar entry, a session list/create screen (name, link name, budget, duration chips) and a host console (5s poll, budget countdown, join QR via `react-native-qrcode-svg`, native share sheet, veto, end-session confirm). A "Party display" button turns the phone into the party screen: fullscreen modal, landscape-first (giant QR left, big Geist-Extrabold type + Electric-Magenta budget countdown right), now-playing + latest requests, pulsing LIVE dot, keep-awake; rotation unlocks ONLY there (new native modules `expo-screen-orientation` + `expo-keep-awake`, app.json orientation → default with an initial PORTRAIT_UP lock).
- **Party Mode: discoverable web home + auto-slugs; Favorites back in the nav** (2026-07-22, `d1a19425`). New `/party` index page (session list + create form) and a "Jam" nav item under Create — visible to everyone, creates 403 server-side for non-studio; the /playlists Jam button routes there. Sessions without a custom link name auto-slug from the title with a 4-char dedupe hash ("Kitchen Party" → `kitchen-party-x3f2`; collisions retry silently, only user-chosen slugs surface 409). Favorites returns as a nav item under My Music (`/library?smartFilter=favorites`) — the Phase-2 chip-only placement buried it (partial revert, see DECISIONS 2026-07-22). **Superseded 2026-07-25:** that query href was a dead click from `/library`; the item now points at the real `/favorites` route (see Fixed).
...
- **Favorites in the sidebar was a dead click** (2026-07-25, `9822c0c4`, root-cause corrected in `49df1b6d`). The nav item pointed at `/library?smartFilter=favorites` (added `d1a19425`), which from `/library` is a same-route soft navigation. `useLibraryFilterState` seeded its reducer from `useSearchParams` exactly **once** and afterwards only ever wrote the URL back (`router.replace`, no equality guard), so an incoming query was invisible to it and the write-back replaced it with the stale state within a tick. The hook now syncs bidirectionally: a `syncedQueryRef` records the last query it wrote or adopted and decides the direction from it, and the push is held until the search debounce has settled. Favorites additionally gets its real route back (`/favorites`, which existed the whole time) because `pathname === href` drives the active highlight and a query href can never match it — Library stayed lit while you were on Favorites. The `/favorites` page itself rendered no `h1` in its empty state and hardcoded English while `favorites.title`/`.noFavorites`/`.noFavoritesDescription` sat unused in all three catalogs; heading always renders now, strings come from next-intl, inline SVG and `gray-*` literals replaced by the Lucide icon and semantic tokens. Smart-filter chips gained `aria-pressed`.
- **Global search navigated to a parameter nobody read** (2026-07-25, `49df1b6d`). Picking a search hit pushed `/library?song=<id>`; no consumer anywhere read that param, and measured in a production build the push from `/library` did not commit at all. It now navigates to the real song route `/library/<id>`.
- **CI security-audit gate unblocked** (2026-07-25, `f67981ed`). Two advisories published overnight turned `pnpm audit --audit-level=high` red — CI step #2, which masks typecheck/lint/tests and blocks the Railway deploy. postcss GHSA-r28c-9q8g-f849 is patched in 8.5.18, so the override moved `>=8.5.10` → `>=8.5.18` (resolves 8.5.23); fully fixed. brace-expansion GHSA-mh99-v99m-4gvg is patched **only** in 5.0.8 while the 1.x/2.x maintenance lines end at 1.1.16/2.1.2, so the 5.x override moved to `>=5.0.8` and the advisory is listed in `pnpm.auditConfig.ignoreGhsas`. Collapsing all three ranges into one `">=5.0.8"` override was tried and is wrong: v5's CJS build exposes no callable default, minimatch 9 dies with `(0, brace_expansion_1.default) is not a function` and `vitest run --coverage` aborts (plain `vitest run` stays green and hides it). Remaining exposure is dev/build tooling only.

- **Mobile: section back-buttons dumped onto the Library tab** (2026-07-22, `c22fae6a` + `f0913234`; device verification pending the next `pnpm release`). Sections opened via the sidebar (Mashup, Personas, Inspire, …) were pushed onto the Library tab's stack regardless of the entry tab — bare hrefs in the 5-way shared route group always resolve into the first group, and the on-device pass proved that even a group-qualified path navigate does not fix it. `goToSection` now dispatches `NAVIGATE` by name directly on the ACTIVE tab's child stack (target = its state key, the `jumpToTab` idiom) — by construction the section can only land on the current tab. Spec updated (`apps/mobile/NAVIGATION.md` rule 1b).
...
- **The client build identifier has been frozen at `7553d92f` (2026-05-17) for 373 commits** (recorded 2026-07-25, `d3f97e2f`/`15d72009`). `NEXT_PUBLIC_BUILD_ID` and `SENTRY_RELEASE` are sticky Railway service variables that only `deploy-production.yml` writes, and that workflow runs on version tags / manual dispatch only. Preferring `RAILWAY_GIT_COMMIT_SHA` in `next.config.mjs` was tried (`49df1b6d`) and is **inert**: the variable does not exist for this service (deployment `c1d163b1` built `49df1b6d` successfully and the release stayed `7553d92f`; `railway variables` lists 14 `RAILWAY_*` names without it). Consequence is telemetry only — error events carry a two-month-old release, which devalues `in_release` pinning during triage. There is **no user-visible harm**: an earlier claim that PWA cache busting was inert and left returning users on a stale shell was retracted after reading `public/sw.js` — navigations are network-first with no cache, `/_next/static` bundles are content-hashed so a new build's filenames never hit the old cache, and `AUDIO_CACHE` is deliberately not namespaced by build id. Open routes: expose the SHA from CI, un-ignore `.git` for a `git rev-parse` in the builder, or accept a unique-per-build id with no Sentry release. None implemented; it is a pipeline decision.
