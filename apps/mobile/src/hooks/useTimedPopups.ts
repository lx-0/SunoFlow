...
// Ported 1:1 from the web player's useTimedPopups: surfaces transient popups when
// playback crosses an item's timestamp (shown for displayDurationMs, then gone).
// Pure React, so it runs unchanged in RN — kept per-app because @sunoflow/core is
// React-free by rule.
